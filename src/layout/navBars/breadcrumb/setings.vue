...
export default {
	...
	methods: {
		...
		// 设置布局切换，重置主题样式（亮色默认，暗色通过开关切换）
		initSetLayoutChange() {
			if (this.$store.state.themeConfig.themeConfig.layout === 'classic') {
				this.onBgColorPickerChange('menuBar', '#E8F0F8');
				this.onBgColorPickerChange('menuBarColor', '#1E293B');
				this.onBgColorPickerChange('topBar', '#FFFFFF');
				this.onBgColorPickerChange('topBarColor', '#1E293B');
			} else if (this.$store.state.themeConfig.themeConfig.layout === 'transverse') {
				this.onBgColorPickerChange('menuBarColor', '#1E293B');
				this.onBgColorPickerChange('topBar', '#E8F0F8');
				this.onBgColorPickerChange('topBarColor', '#1E293B');
